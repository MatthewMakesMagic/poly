...
async function run() {
  const windowClose = '2026-02-06T11:15:00Z';
  const finalStart = '2026-02-06T11:14:00Z';
  const windowOpen = '2026-02-06T11:00:00Z';
  ...
  console.log("=== 06:15 ET WINDOW: FINAL 60 SECONDS — PRICE FEEDS ===\n");
  ...
  console.log("\n\n=== 06:15 ET WINDOW: FINAL 60 SECONDS — CLOB ===\n");
  ...
  // Query 3: Threshold — Binance vs Chainlink direction (fixed cross-join)
  ...
  console.log("\n\n=== 06:15 ET: THRESHOLD ANALYSIS (final 30s) ===\n");
  ...
  // Query 4: Threshold crossings across entire window
  const crossing = await pool.query(`
    ...
    bin_open AS (
      SELECT AVG(price) AS op FROM binance WHERE sec BETWEEN $1::timestamptz AND ($1::timestamptz + INTERVAL '10 seconds')
    ),
    cl_open AS (
      SELECT AVG(price) AS op FROM chainlink WHERE sec BETWEEN $1::timestamptz AND ($1::timestamptz + INTERVAL '10 seconds')
    ),
    bin_dir AS (
      SELECT sec, 
    ...
    SELECT 'Binance' AS feed, to_char(sec AT TIME ZONE 'America/New_York', 'HH24:MI:SS') AS cross_time_et, prev_dir || ' -> ' || dir AS crossing
    FROM bin_dir WHERE dir != prev_dir AND prev_dir IS NOT NULL
    UNION ALL
    SELECT 'Chainlink' AS feed, to_char(sec AT TIME ZONE 'America/New_York', 'HH24:MI:SS') AS cross_time_et, prev_dir || ' -> ' || dir AS crossing
    FROM cl_dir WHERE dir != prev_dir AND prev_dir IS NOT NULL
    ORDER BY cross_time_et
  `, [windowOpen, windowClose]);
  
  console.log("\n\n=== 06:15 ET: THRESHOLD CROSSINGS (entire window) ===\n");
  console.log("Every time Binance or Chainlink crossed the window-open price (direction flip)\n");
  console.log("FEED       CROSS_TIME_ET  CROSSING");
  console.log("-".repeat(45));
  for (const r of crossing.rows) {
    console.log(
      String(r.feed).padEnd(11) +
      String(r.cross_time_et).padEnd(15) +
      r.crossing
    );
  }
