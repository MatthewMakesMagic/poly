/**
 * Paired MM: Strict Position Matching
 *
 * Like mm-paired-polyref but enforces strict position balance:
 * - Tracks imbalance = abs(upTokens - downTokens) / max(upTokens + downTokens, 1)
 * - Only allows edge buys on the dominant side if imbalance < 0.20 (20%)
 * - The underweight side can always edge-buy
 * - Hedge entry unchanged
 *
 * S = polyRef.price for BS computation.
 */

export const name = 'mm-paired-strict';
export const description = 'Paired MM: strict position matching, blocks dominant-side buys when imbalance > 20%.';
...
export function evaluate(state, config) {
  const {
    ...
    maxImbalance = defaults.maxImbalance,
  } = config;
  ...
  const totalTokens = upTokens + downTokens;
  const imbalance = totalTokens > 0 ? Math.abs(upTokens - downTokens) / totalTokens : 0;
  const upDominant = upTokens > downTokens;
  const downDominant = downTokens > upTokens;

  // ─── MODE 1: EDGE ENTRY — with strict imbalance constraints ───

  // UP edge buy: blocked if UP is dominant and imbalance >= maxImbalance
  const upBlocked = upDominant && imbalance >= maxImbalance;
  if (!upBlocked
    && askUp < fairUp - minEdge
    && askUp <= maxEntryPrice
    && askUp > 0.01
    && upCost < maxPerSide
    ...
    signals.push({
      ...
      reason: `edge_buy: UP, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}, imbal=${imbalance.toFixed(2)}`,
    });
  ...
  // DOWN edge buy: blocked if DOWN is dominant and imbalance >= maxImbalance
  const downBlocked = downDominant && imbalance >= maxImbalance;
  if (!downBlocked
    && askDown < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `edge_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}, imbal=${imbalance.toFixed(2)}`,
    });
  ...
  // ─── MODE 2: HEDGE ENTRY — buy the other side at pair-breakeven ───
