/**
 * Paired MM: Asymmetric Edge Thresholds
 *
 * Like mm-paired-polyref but with separate edge thresholds for UP and DOWN:
 * - minEdgeUp default = 0.01 (looser for UP, more likely to buy UP)
 * - minEdgeDown default = 0.03 (tighter for DOWN, more selective)
 *
 * S = polyRef.price for BS computation.
 */

export const name = 'mm-paired-asymmetric';
export const description = 'Paired MM: asymmetric edge thresholds (loose UP, tight DOWN) + position-aware hedging.';
...
export const sweepGrid = {
  minEdgeUp: [0.005, 0.01, 0.02, 0.03],
  minEdgeDown: [0.01, 0.02, 0.03, 0.05],
  maxPerSide: [6, 10, 20],
...
export function evaluate(state, config) {
  const {
    minEdgeUp = defaults.minEdgeUp,
    minEdgeDown = defaults.minEdgeDown,
  ...
  // ─── MODE 1: EDGE ENTRY — asymmetric thresholds ───

  if (askUp < fairUp - minEdgeUp
    && askUp <= maxEntryPrice
    && askUp > 0.01
    && upCost < maxPerSide
    ...
    signals.push({
      ...
      reason: `edge_buy: UP, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}, edge=${(fairUp - askUp).toFixed(3)}, thresh=${minEdgeUp}`,
    });
  ...
  if (askDown < fairDown - minEdgeDown
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `edge_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}, edge=${(fairDown - askDown).toFixed(3)}, thresh=${minEdgeDown}`,
    });
