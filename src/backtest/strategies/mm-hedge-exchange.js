/**
 * Continuous MM: Exchange Median
 *
 * True continuous market maker using exchange median as S.
 * Exchange median leads CL by 2-5s — real MMs use exchange data, not the lagged oracle.
 *
 * S = state.getExchangeMedian() (binance, coinbase, kraken, bybit, okx)
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-hedge-exchange';
export const description = 'Continuous MM: BS fair from exchange median. Buy both sides whenever cheap.';
...
export function evaluate(state, config) {
  ...
  // Track CL for vol estimation (vol always from CL)
  if (chainlink?.price) {
  ...
  if (clobUp.bestAsk < fairUp - minEdge
    ...
    signals.push({
      ...
      reason: `mm_buy: UP, fair=${fairUp.toFixed(3)}, ask=${clobUp.bestAsk.toFixed(3)}, edge=${(fairUp - clobUp.bestAsk).toFixed(3)}, exchMed=$${exchangeMedian.toFixed(0)}`,
    });
  ...
  if (clobDown.bestAsk < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `mm_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${clobDown.bestAsk.toFixed(3)}, edge=${(fairDown - clobDown.bestAsk).toFixed(3)}, exchMed=$${exchangeMedian.toFixed(0)}`,
    });
