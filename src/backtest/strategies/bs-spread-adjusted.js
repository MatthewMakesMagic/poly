 ...
 * BS Spread Adjusted
 *
 * Adjusts the mispricing threshold by CLOB spread width.
 * Wide CLOB spreads → higher execution cost → need larger mispricing to be profitable.
 * Narrow spreads → lower cost → smaller mispricings become tradeable.
 *
 * effective_threshold = base_threshold + spread_multiplier * clob_spread
 *
 * This naturally filters out low-liquidity moments when spreads blow out.
 */

export const name = 'bs-spread-adjusted';
export const description = 'BS pricing with mispricing threshold adjusted by CLOB spread width. Wider spreads require larger mispricings.';

export const defaults = {
  baseMispricingThreshold: 0.12,
  spreadMultiplier: 2.0,       // Each $0.01 of spread adds $0.02 to threshold
  entryWindowMs: 90000,
  maxEntryPrice: 0.60,
  capitalPerTrade: 2,
  minVolSamples: 10,
  maxSpread: 0.15,             // Don't trade if spread > 15 cents
  windowDurationMs: 900000,
};

export const sweepGrid = {
  baseMispricingThreshold: [0.08, 0.12, 0.15, 0.20],
  spreadMultiplier: [1.0, 2.0, 3.0],
...
export function evaluate(state, config) {
  if (hasBought) return [];
  const {
    baseMispricingThreshold = defaults.baseMispricingThreshold,
    spreadMultiplier = defaults.spreadMultiplier,
  ...
  // Check CLOB spread
  const upSpread = clobUp.spread;
  const downSpread = clobDown.spread;
  if (upSpread > maxSpread || downSpread > maxSpread) return [];

  // Dynamic threshold based on spread
  const relevantSpread = Math.max(upSpread, downSpread);
  const mispricingThreshold = baseMispricingThreshold + spreadMultiplier * relevantSpread;
  ...
  if (mispricing > mispricingThreshold && clobDown.bestAsk <= maxEntryPrice) {
    ...
    return [{ action: 'buy', token: `${win.symbol}-down`, capitalPerTrade,
      reason: `bs_spread: P(UP)=${bsFairUp.toFixed(3)}, CLOB=${clobUpPrice.toFixed(3)}, gap=${mispricing.toFixed(3)}, thresh=${mispricingThreshold.toFixed(3)}` }];
  }
