/**
 * Continuous MM: PolyRef Basis
 *
 * True continuous market maker using Polymarket's composite reference price as S.
 * PolyRef was our best directional model in v1 (+$31.80 directional, +$132.75 MM).
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-hedge-polyref';
export const description = 'Continuous MM: BS fair from PolyRef. Buy both sides whenever cheap.';
...
export function evaluate(state, config) {
  ...
  if (clobUp.bestAsk < fairUp - minEdge
    ...
    signals.push({
      ...
      reason: `mm_buy: UP, fair=${fairUp.toFixed(3)}, ask=${clobUp.bestAsk.toFixed(3)}, edge=${(fairUp - clobUp.bestAsk).toFixed(3)}, polyRef=$${polyRef.price.toFixed(0)}`,
    });
  ...
  if (clobDown.bestAsk < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `mm_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${clobDown.bestAsk.toFixed(3)}, edge=${(fairDown - clobDown.bestAsk).toFixed(3)}, polyRef=$${polyRef.price.toFixed(0)}`,
    });
