/**
 * Rebalance MM: Aggressive Position Rebalancing
 *
 * Like mm-paired-polyref but aggressively rebalances:
 * - If upTokens > 2 * downTokens: ONLY buy DOWN (skip UP edge buys)
 * - If downTokens > 2 * upTokens: ONLY buy UP (skip DOWN edge buys)
 * - If within 2x ratio: normal edge buying on both sides
 * - Double capitalPerEntry on the underweight side
 *
 * S = polyRef.price for BS computation.
 */

export const name = 'mm-rebalance-polyref';
export const description = 'Rebalance MM: aggressive position rebalancing with doubled capital on underweight side.';
...
export function evaluate(state, config) {
  ...
  const heavyUp = upTokens > 2 * downTokens && upTokens > 0;
  const heavyDown = downTokens > 2 * upTokens && downTokens > 0;

  // ─── MODE 1: EDGE ENTRY — with rebalance constraints ───

  // UP edge buy: skip if heavy UP
  if (!heavyUp
    ...
    const cap = heavyDown ? capitalPerEntry * 2 : capitalPerEntry;
    const tokens = cap / askUp;
    signals.push({
      action: 'buy',
      token: `${sym}-up`,
      capitalPerTrade: cap,
      reason: `edge_buy: UP${heavyDown ? ' (2x rebal)' : ''}, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}`,
    });
    upCost += cap;
    upTokens += tokens;
    lastUpBuyMs = nowMs;
  ...
  // DOWN edge buy: skip if heavy DOWN
  if (!heavyDown
    ...
    const cap = heavyUp ? capitalPerEntry * 2 : capitalPerEntry;
    const tokens = cap / askDown;
    signals.push({
      action: 'buy',
      token: `${sym}-down`,
      capitalPerTrade: cap,
      reason: `edge_buy: DOWN${heavyUp ? ' (2x rebal)' : ''}, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}`,
    });
    downCost += cap;
    downTokens += tokens;
    lastDownBuyMs = nowMs;
