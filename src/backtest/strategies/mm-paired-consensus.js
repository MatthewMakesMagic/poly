/**
 * Paired MM: Consensus Gate
 *
 * Same as mm-paired-polyref but with exchange consensus check:
 * If exchange spread rangePct > 0.1%, skip ALL buying (exchanges disagreeing).
 * When spread is tight or unavailable, normal operation.
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-consensus';
export const description = 'Paired MM: PolyRef BS + hedging gated by exchange consensus (skips when exchanges disagree >0.1%).';
...
export const sweepGrid = {
  ...
  maxExchangeSpreadPct: [0.0005, 0.001, 0.002],
};
...
export function evaluate(state, config) {
  ...
  // Consensus gate: skip if exchanges are disagreeing
  const exchangeSpread = state.getExchangeSpread?.();
  if (exchangeSpread?.rangePct > maxExchangeSpreadPct) return [];
