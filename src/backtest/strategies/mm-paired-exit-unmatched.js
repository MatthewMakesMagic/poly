/**
 * Paired MM: Exit Unmatched Tokens
 *
 * Same as mm-paired-polyref for normal entry (edge + hedge).
 * When timeToCloseMs <= 30s, sells all unmatched (excess) tokens
 * on the dominant side to avoid holding naked directional risk into resolution.
 *
 * Tracks individual entries with prices and sizes so sell signals
 * can reference specific positions.
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-exit-unmatched';
export const description = 'Paired MM with late-window exit of unmatched tokens. Sells excess side at T-30s to avoid naked directional risk.';
...
export const sweepGrid = {
  ...
  exitThresholdMs: [15000, 30000, 60000],
};
...
export function evaluate(state, config) {
  ...
  // EXIT: sell unmatched tokens when close to resolution
  if (win.timeToCloseMs <= exitThresholdMs && !exitSellsDone) {
    exitSellsDone = true;
    if (upTokens > downTokens) {
      const excessTokens = upTokens - downTokens;
      const excessEntries = Math.ceil(excessTokens / (capitalPerEntry / 0.50)); // approximate
      for (let i = 0; i < upEntries.length && i < excessEntries; i++) {
        signals.push({
          action: 'sell',
          token: `${sym}-up`,
          size: upEntries[i].tokens,
          reason: `exit_unmatched: UP, entry=${upEntries[i].price.toFixed(3)}, excess=${excessTokens.toFixed(1)}`,
        });
      }
    } else if (downTokens > upTokens) {
      const excessTokens = downTokens - upTokens;
      const excessEntries = Math.ceil(excessTokens / (capitalPerEntry / 0.50));
      for (let i = 0; i < downEntries.length && i < excessEntries; i++) {
        signals.push({
          action: 'sell',
          token: `${sym}-down`,
          size: downEntries[i].tokens,
          reason: `exit_unmatched: DOWN, entry=${downEntries[i].price.toFixed(3)}, excess=${excessTokens.toFixed(1)}`,
        });
      }
    }
  ...
  if (askUp < fairUp - minEdge
    ...
    upTokens += tokens;
    upEntries.push({ price: askUp, tokens });
    lastUpBuyMs = nowMs;
  }
  ...
  if (askDown < fairDown - minEdge
    ...
    downTokens += tokens;
    downEntries.push({ price: askDown, tokens });
    lastDownBuyMs = nowMs;
  }
