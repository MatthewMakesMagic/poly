/**
 * Paired MM: Inverse Confidence Sizing
 *
 * Like mm-paired-polyref but capitalPerEntry scales INVERSELY with distance from K:
 * - distance = abs(chainlink.price - K) / K
 * - scaledCapital = capitalPerEntry * Math.max(0.5, 1 - distance * 10) — floored at 0.5x
 *
 * Near K = uncertain = best matching opportunity (both sides cheap). Far from K = smaller buys.
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-invconfidence';
export const description = 'Paired MM with inverse confidence sizing. Buys bigger near strike (best pairing opportunity), smaller far from strike.';
...
export function evaluate(state, config) {
  const {
    ...
    minCapMultiplier = defaults.minCapMultiplier,
  ...
  // Inverse confidence sizing: scale capital DOWN with distance from K
  const currentPrice = chainlink?.price || polyRef.price;
  const distance = Math.abs(currentPrice - K) / K;
  const scaledCapital = capitalPerEntry * Math.max(minCapMultiplier, 1 - distance * distanceScale);
