/**
 * Paired MM: Kelly Sizing
 *
 * Like mm-paired-polyref but sizes entries using Kelly criterion:
 * - pWin = bsFair for UP tokens, (1-bsFair) for DOWN tokens
 * - payoff = (1.00 / askPrice) - 1 (net payoff ratio if win)
 * - f = (pWin * payoff - (1 - pWin)) / payoff — Kelly fraction
 * - scaledCapital = capitalPerEntry * clamp(f, 0.25, 2.0)
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-kelly';
export const description = 'Paired MM with Kelly criterion sizing. Sizes entries proportional to edge/odds ratio for optimal growth.';
...
export function evaluate(state, config) {
  const {
    ...
    minKellyFrac = defaults.minKellyFrac,
    maxKellyFrac = defaults.maxKellyFrac,
  } = config;
  ...
  // MODE 1: EDGE ENTRY with Kelly sizing
  if (askUp < fairUp - minEdge
    ...
    const pWin = fairUp;
    const payoff = (1.00 / askUp) - 1;
    const f = payoff > 0 ? (pWin * payoff - (1 - pWin)) / payoff : 0;
    const kellyFrac = Math.max(minKellyFrac, Math.min(f, maxKellyFrac));
    const scaledCapital = capitalPerEntry * kellyFrac;
    const tokens = scaledCapital / askUp;
    signals.push({
      action: 'buy',
      token: `${sym}-up`,
      capitalPerTrade: scaledCapital,
      reason: `edge_buy: UP, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}, kelly=${kellyFrac.toFixed(2)}`,
    });
  ...
  if (askDown < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: scaledCapital,
      reason: `edge_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}, kelly=${kellyFrac.toFixed(2)}`,
    });
  ...
  // MODE 2: HEDGE ENTRY (uses base capitalPerEntry, not Kelly)
