/**
 * Paired MM: PolyRef with Position-Aware Hedging
 *
 * True market maker that ensures nearly all tokens are paired (hedged).
 *
 * Two entry modes:
 * 1. EDGE ENTRY: Buy either side when cheap per BS fair value (same as mm-hedge-polyref)
 * 2. HEDGE ENTRY: When holding unmatched tokens on one side, buy the other side
 *    at any price where the PAIR is profitable: askOther < 1.00 - avgEntryPrice - minPairEdge
 *    This is much more permissive than the BS filter — it fires even when BS says
 *    the other side is "expensive."
 *
 * The hedge entry ensures most tokens end up paired, converting directional
 * binary bets into guaranteed spread capture ($1.00 - combined cost per pair).
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-polyref';
export const description = 'Paired MM: PolyRef BS + position-aware hedging. Buys the other side at pair-breakeven to maximize matching.';
...
export const defaults = {
  ...
  capitalPerEntry: 2,
  cooldownMs: 10000,
  minPairEdge: 0.02,       // min profit per pair to trigger hedge ($0.02 = 2c)
  entryWindowMs: 300000,
  exitWindowMs: 5000,
  maxEntryPrice: 0.65,
  maxHedgePrice: 0.65,     // max price for hedge side (can be more permissive)
  minVolSamples: 10,
...
export function evaluate(state, config) {
  ...
  // ─── MODE 1: EDGE ENTRY — buy either side if cheap per BS fair ───
  ...
  // Only fires if we have unmatched exposure AND the edge entry didn't already buy this side

  if (upTokens > downTokens && downCost < maxPerSide && nowMs - lastDownBuyMs >= cooldownMs) {
    // We're long UP, need DOWN to hedge
  ...
  if (downTokens > upTokens && upCost < maxPerSide && nowMs - lastUpBuyMs >= cooldownMs) {
    // We're long DOWN, need UP to hedge
