/**
 * Paired MM: Time Decay Hedge
 *
 * Like mm-paired-polyref but minPairEdge decreases linearly as window closes:
 * - At entryWindowMs (T-5min): effectiveMinPairEdge = 0.05
 * - At exitWindowMs (T-5s): effectiveMinPairEdge = 0.0
 * - Linear interpolation between
 *
 * Makes hedge entries increasingly aggressive as time runs out,
 * ensuring more tokens get paired even at thin margins.
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-timedecay';
export const description = 'Paired MM with time-decaying hedge threshold. Hedge margin decreases linearly toward close for aggressive late pairing.';
...
export const sweepGrid = {
  ...
  capitalPerEntry: [2],
  earlyPairEdge: [0.03, 0.05, 0.08],
  latePairEdge: [0.0, 0.01],
};
...
export function evaluate(state, config) {
  const {
    ...
    earlyPairEdge = defaults.earlyPairEdge,
    latePairEdge = defaults.latePairEdge,
  } = config;
  ...
  // Time decay: linearly interpolate minPairEdge from earlyPairEdge to latePairEdge
  const timeRange = entryWindowMs - exitWindowMs;
  const timeElapsed = entryWindowMs - win.timeToCloseMs;
  const timeFraction = Math.max(0, Math.min(1, timeElapsed / timeRange));
  const effectiveMinPairEdge = earlyPairEdge + (latePairEdge - earlyPairEdge) * timeFraction;
  ...
  if (upTokens > downTokens && downCost < maxPerSide && nowMs - lastDownBuyMs >= cooldownMs) {
    ...
    if (askDown > 0.01 && askDown < maxDownPrice && askDown <= maxHedgePrice) {
      ...
      signals.push({
        ...
        token: `${sym}-down`,
        capitalPerTrade: capitalPerEntry,
        reason: `hedge_buy: DOWN, avgUp=${avgUpPrice.toFixed(3)}, askDn=${askDown.toFixed(3)}, pairEdge=${effectiveMinPairEdge.toFixed(3)}`,
      });
      downCost += capitalPerEntry;
      downTokens += tokens;
      lastDownBuyMs = nowMs;
  ...
  if (downTokens > upTokens && upCost < maxPerSide && nowMs - lastUpBuyMs >= cooldownMs) {
    ...
    const maxUpPrice = 1.00 - avgDownPrice - effectiveMinPairEdge;
    if (askUp > 0.01 && askUp < maxUpPrice && askUp <= maxHedgePrice) {
      ...
      signals.push({
        ...
        token: `${sym}-up`,
        capitalPerTrade: capitalPerEntry,
        reason: `hedge_buy: UP, avgDn=${avgDownPrice.toFixed(3)}, askUp=${askUp.toFixed(3)}, pairEdge=${effectiveMinPairEdge.toFixed(3)}`,
      });
      upCost += capitalPerEntry;
      upTokens += tokens;
      lastUpBuyMs = nowMs;
