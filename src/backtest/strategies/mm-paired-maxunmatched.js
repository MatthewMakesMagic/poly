/**
 * Paired MM: Capped Unmatched Exposure
 *
 * Like mm-paired-polyref but adds a hard cap on unmatched (directional) exposure:
 * - maxUnmatchedCost (default $4) caps the cost difference between sides
 * - When unmatchedCost >= maxUnmatchedCost, only allow buys on the underweight side
 * - Prevents runaway directional risk while still capturing spread
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-maxunmatched';
export const description = 'Paired MM with hard cap on unmatched exposure. Blocks dominant-side buys when directional imbalance exceeds threshold.';
...
export function evaluate(state, config) {
  const {
    ...
    maxUnmatchedCost = defaults.maxUnmatchedCost,
  } = config;
  ...
  // Check unmatched exposure
  const unmatchedCost = Math.abs(upCost - downCost);
  const upIsHeavy = upCost > downCost;
  const downIsHeavy = downCost > upCost;
  const atUnmatchedCap = unmatchedCost >= maxUnmatchedCost;

  // MODE 1: EDGE ENTRY — skip dominant side if at unmatched cap
  if (askUp < fairUp - minEdge
    && askUp <= maxEntryPrice
    && askUp > 0.01
    && upCost < maxPerSide
    && nowMs - lastUpBuyMs >= cooldownMs
    && !(atUnmatchedCap && upIsHeavy)) {
    const tokens = capitalPerEntry / askUp;
    signals.push({
      action: 'buy',
      token: `${sym}-up`,
      capitalPerTrade: capitalPerEntry,
      reason: `edge_buy: UP, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}, unmatched=$${unmatchedCost.toFixed(1)}`,
    });
  ...
  if (askDown < fairDown - minEdge
    ...
    && !(atUnmatchedCap && downIsHeavy)) {
    const tokens = capitalPerEntry / askDown;
    signals.push({
      action: 'buy',
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `edge_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}, unmatched=$${unmatchedCost.toFixed(1)}`,
    });
