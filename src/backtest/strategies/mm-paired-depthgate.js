/**
 * Paired MM: Depth Gated
 *
 * Same as mm-paired-polyref but with CLOB depth check:
 * Only buy UP if clobUp.askSize >= minAskSize
 * Only buy DOWN if clobDown.askSize >= minAskSize
 * Avoids thin book fills that may not be realistic in production.
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-depthgate';
export const description = 'Paired MM: PolyRef BS + hedging gated by CLOB depth (skips thin books for realistic fills).';
...
export function evaluate(state, config) {
  const {
    ...
    minAskSize = defaults.minAskSize,
  } = config;
  ...
  const askUpSize = clobUp.askSize || 0;
  const askDownSize = clobDown.askSize || 0;

  // ─── MODE 1: EDGE ENTRY (depth gated) ───

  if (askUp < fairUp - minEdge
    && askUp <= maxEntryPrice
    && askUp > 0.01
    && askUpSize >= minAskSize
    && upCost < maxPerSide
    ...
    signals.push({
      ...
      reason: `edge_buy: UP, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}, depth=${askUpSize}`,
    });
  ...
  if (askDown < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `edge_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}, depth=${askDownSize}`,
    });
  ...
  // ─── MODE 2: HEDGE ENTRY (depth gated) ───
  ...
  if (upTokens > downTokens && downCost < maxPerSide && nowMs - lastDownBuyMs >= cooldownMs) {
    ...
    if (askDown > 0.01
      ...
      signals.push({
        ...
        token: `${sym}-down`,
        capitalPerTrade: capitalPerEntry,
        reason: `hedge_buy: DOWN, avgUp=${avgUpPrice.toFixed(3)}, askDn=${askDown.toFixed(3)}, depth=${askDownSize}`,
      });
      downCost += capitalPerEntry;
      downTokens += tokens;
      lastDownBuyMs = nowMs;
  ...
  if (downTokens > upTokens && upCost < maxPerSide && nowMs - lastUpBuyMs >= cooldownMs) {
    ...
    if (askUp > 0.01
      ...
      signals.push({
        ...
        token: `${sym}-up`,
        capitalPerTrade: capitalPerEntry,
        reason: `hedge_buy: UP, avgDn=${avgDownPrice.toFixed(3)}, askUp=${askUp.toFixed(3)}, depth=${askUpSize}`,
      });
      upCost += capitalPerEntry;
      upTokens += tokens;
      lastUpBuyMs = nowMs;
