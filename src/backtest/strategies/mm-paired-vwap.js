/**
 * Paired MM: VWAP Blend with Position-Aware Hedging
 *
 * Same as mm-paired-polyref but uses a blended price source:
 * S = 0.6 * polyRef.price + 0.4 * exchangeMedian
 * Falls back to polyRef.price if exchangeMedian is unavailable.
 */

export const name = 'mm-paired-vwap';
export const description = 'Paired MM: VWAP blend (60% polyRef + 40% exchange) BS + position-aware hedging.';
...
export function evaluate(state, config) {
  ...
  // Blended price: 60% polyRef + 40% exchange median (fallback to polyRef)
  const exchangeMedian = state.getExchangeMedian();
  const S = exchangeMedian
    ? 0.6 * polyRef.price + 0.4 * exchangeMedian
    : polyRef.price;
