/**
 * Two-Way Quoting MM: PolyRef
 *
 * Buy at ask AND sell at bid. Tracks individual positions per side.
 * - Buy UP when askUp < fairUp - minEdge
 * - Sell UP when bestBid > entryPrice + minProfit
 * - After selling UP, buy DOWN to hedge at whatever price
 * - Vice versa for DOWN
 *
 * S = polyRef.price
...
export const name = 'mm-twoway-polyref';
export const description = 'Two-way quoting MM: buys at ask, sells at bid when profitable, then hedges the other side.';
...
export function evaluate(state, config) {
  ...
  // ─── SELL: Check if any positions can be sold profitably ───
    ...
    for (let i = 0; i < upPositions.length; i++) {
      if (bidUp > upPositions[i].entryPrice + minProfit) {
        signals.push({
          ...
          reason: `profit_sell: UP, entry=${upPositions[i].entryPrice.toFixed(3)}, bid=${bidUp.toFixed(3)}, profit=${(bidUp - upPositions[i].entryPrice).toFixed(3)}`,
        });
    ...
    for (let i = 0; i < downPositions.length; i++) {
      if (bidDown > downPositions[i].entryPrice + minProfit) {
        signals.push({
          ...
          reason: `profit_sell: DOWN, entry=${downPositions[i].entryPrice.toFixed(3)}, bid=${bidDown.toFixed(3)}, profit=${(bidDown - downPositions[i].entryPrice).toFixed(3)}`,
        });
  ...
  // ─── HEDGE after sell: buy the other side ───

  if (soldUp && askDown > 0.01 && askDown <= maxHedgePrice && downTotalCost < maxPerSide) {
    const tokens = capitalPerEntry / askDown;
    signals.push({
      action: 'buy',
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `post_sell_hedge: DOWN after selling UP, askDn=${askDown.toFixed(3)}`,
    });
  ...
  if (soldDown && askUp > 0.01 && askUp <= maxHedgePrice && upTotalCost < maxPerSide) {
    const tokens = capitalPerEntry / askUp;
    signals.push({
      action: 'buy',
      token: `${sym}-up`,
      capitalPerTrade: capitalPerEntry,
      reason: `post_sell_hedge: UP after selling DOWN, askUp=${askUp.toFixed(3)}`,
    });
  ...
  // ─── BUY: Edge entry (same as template) ───
  ...
  // ─── HEDGE: Position-aware hedging (same logic as template) ───
