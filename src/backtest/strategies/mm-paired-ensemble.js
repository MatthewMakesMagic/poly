/**
 * Paired MM: Ensemble S (PolyRef + Exchange Median)
 *
 * Same as mm-paired-polyref but uses blended S for BS computation:
 * S = 0.6 * polyRef.price + 0.4 * exchangeMedian
 * If exchangeMedian is null, uses polyRef.price alone.
 *
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-ensemble';
export const description = 'Paired MM: BS with ensemble S (60% PolyRef + 40% exchange median) for more robust fair value.';
...
export function evaluate(state, config) {
  const {
    ...
    polyRefWeight = defaults.polyRefWeight,
  } = config;
  ...
  // Ensemble S: blend polyRef with exchange median
  const exchangeMedian = state.getExchangeMedian?.();
  const S = exchangeMedian
    ? polyRefWeight * polyRef.price + (1 - polyRefWeight) * exchangeMedian
    : polyRef.price;

  const bsFair = computeBSFair(S, K, clHistory, win.timeToCloseMs);
  ...
  if (askUp < fairUp - minEdge
    ...
    signals.push({
      ...
      reason: `edge_buy: UP, fair=${fairUp.toFixed(3)}, ask=${askUp.toFixed(3)}, S=${S.toFixed(1)}`,
    });
  ...
  if (askDown < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `edge_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${askDown.toFixed(3)}, S=${S.toFixed(1)}`,
    });
