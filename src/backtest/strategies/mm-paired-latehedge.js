/**
 * Paired MM: Late Aggressive Hedge
 *
 * Two phases:
 * - Phase 1 (timeToCloseMs between entryWindowMs and 120000): Normal edge + hedge buying
 * - Phase 2 (timeToCloseMs between 120000 and exitWindowMs): HEDGE ONLY mode.
 *   No edge buys. minPairEdge=0 (accept any hedge that doesn't lose money).
 *   More aggressive maxHedgePrice=0.75.
 *
 * S = polyRef.price
 * Vol estimated from CL history (settlement oracle).
 */

export const name = 'mm-paired-latehedge';
export const description = 'Paired MM: Normal edge+hedge early, then aggressive hedge-only in final 2 min (minPairEdge=0, maxHedge=0.75).';
...
export const sweepGrid = {
  ...
  latePhaseMs: [60000, 120000, 180000],
};
...
export function evaluate(state, config) {
  const {
    ...
    lateHedgePrice = defaults.lateHedgePrice,
    latePhaseMs = defaults.latePhaseMs,
  ...
  const isLatePhase = win.timeToCloseMs < latePhaseMs;

  // ─── PHASE 1: NORMAL EDGE + HEDGE (before late phase) ───
  ...
  // ─── PHASE 2: LATE AGGRESSIVE HEDGE ONLY (no edge buys) ───

  if (isLatePhase) {
    // Hedge with minPairEdge=0 and lateHedgePrice cap
    if (upTokens > downTokens && downCost < maxPerSide && nowMs - lastDownBuyMs >= cooldownMs) {
      const avgUpPrice = upCost / upTokens;
      const maxDownPrice = 1.00 - avgUpPrice; // minPairEdge=0
      if (askDown > 0.01 && askDown < maxDownPrice && askDown <= lateHedgePrice) {
        const tokens = capitalPerEntry / askDown;
        signals.push({
          action: 'buy',
          token: `${sym}-down`,
          capitalPerTrade: capitalPerEntry,
          reason: `late_hedge: DOWN, avgUp=${avgUpPrice.toFixed(3)}, askDn=${askDown.toFixed(3)}, pairProfit=${(1.00 - avgUpPrice - askDown).toFixed(3)}`,
        });
        downCost += capitalPerEntry;
        downTokens += tokens;
        lastDownBuyMs = nowMs;
    ...
    if (downTokens > upTokens && upCost < maxPerSide && nowMs - lastUpBuyMs >= cooldownMs) {
      const avgDownPrice = downCost / downTokens;
      const maxUpPrice = 1.00 - avgDownPrice; // minPairEdge=0
      if (askUp > 0.01 && askUp < maxUpPrice && askUp <= lateHedgePrice) {
        const tokens = capitalPerEntry / askUp;
        signals.push({
          action: 'buy',
          token: `${sym}-up`,
          capitalPerTrade: capitalPerEntry,
          reason: `late_hedge: UP, avgDn=${avgDownPrice.toFixed(3)}, askUp=${askUp.toFixed(3)}, pairProfit=${(1.00 - avgDownPrice - askUp).toFixed(3)}`,
        });
        upCost += capitalPerEntry;
        upTokens += tokens;
        lastUpBuyMs = nowMs;
