/**
 * Continuous MM: VWAP-Predicted CL
 *
 * True continuous market maker using predicted CL as S.
 * Models the CoinGecko/BraveNewCoin VWAP pipeline that CL actually uses.
 * CL absorbs 65% of the exchange-CL gap in ~3s (measured transfer function).
 *
 * S = chainlink.price + (exchangeMedian - chainlink.price) * absorptionRate
 * Vol estimated from CL history.
 */

export const name = 'mm-hedge-vwap';
export const description = 'Continuous MM: BS fair from VWAP-predicted CL. Buy both sides whenever cheap.';
...
export function evaluate(state, config) {
  ...
  // Predicted CL = current CL + absorption of exchange-CL gap
  const predictedCL = chainlink.price + (exchangeMedian - chainlink.price) * absorptionRate;

  const bsFair = computeBSFair(predictedCL, K, clHistory, win.timeToCloseMs);
  ...
  if (clobUp.bestAsk < fairUp - minEdge
    ...
    signals.push({
      ...
      reason: `mm_buy: UP, fair=${fairUp.toFixed(3)}, ask=${clobUp.bestAsk.toFixed(3)}, edge=${(fairUp - clobUp.bestAsk).toFixed(3)}, predCL=$${predictedCL.toFixed(0)}`,
    });
  ...
  if (clobDown.bestAsk < fairDown - minEdge
    ...
    signals.push({
      ...
      token: `${sym}-down`,
      capitalPerTrade: capitalPerEntry,
      reason: `mm_buy: DOWN, fair=${fairDown.toFixed(3)}, ask=${clobDown.bestAsk.toFixed(3)}, edge=${(fairDown - clobDown.bestAsk).toFixed(3)}, predCL=$${predictedCL.toFixed(0)}`,
    });
